...
class WifiManager:
    ...
    def _scan_linux(self) -> set:
        try:
            # Принудительное сканирование перед list
...
async def run_calibration(config_path: str = "appsettings.json"):
    """Полная калибровка: фаза 1 (обнаружение) + фаза 2 (привязка SSID)."""
    ...
    # Сначала читаем из файла чтобы получить строку подключения
    ...
    # Перезагружаем конфигурацию: БД > файл > defaults
    async with db_session_factory() as session:
    ...
    if not cameras:
        ...
        # Выставляем IsActive = true для всех камер
        async with db_session_factory() as session:
